...
console.log("");
// ¿cuánto dinero está DE VERDAD aparcado, de media, y por tanto cuánto cuesta el ETF al año?
let suma = 0, nd = 0, ventas = 0;
{ const etf = "SPY";
  let caja = 60000, acc = 0, ab = [];
  const porDia = new Map();
  for (const x of T) { if (!porDia.has(x.dC)) porDia.set(x.dC, []); porDia.get(x.dC).push(x); }
  for (const hoy of DIAS) {
    const p = P[etf].get(hoy);
    acc *= (1 + Math.pow(1.013, 1 / 252) - 1);
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) { caja += ab[i].dinero * ab[i].mult; ab.splice(i, 1); }
    const inv = () => ab.reduce((a, b) => a + b.dinero, 0);
    for (const x of (porDia.get(hoy) || [])) {
      if (ab.length >= 4) continue;
      ...
      if (falta > 0 && acc > 0) { const v = Math.min(acc, falta / p); acc -= v; caja += v * p; ventas++; }
      const n = Math.floor(Math.min(tope, caja) / (x.ask * 100));
      if (n < 1) continue;
...
const medio = suma / nd;
console.log("");
console.log("  ══ ¿CUÁNTO CUESTA DE VERDAD EL ETF? ══");
console.log("");
console.log("  saldo MEDIO aparcado en el ETF, 2021-2026:  " + D(medio));
console.log("  veces que se vendió ETF para financiar una señal: " + ventas + "  (" + (ventas / 5.63).toFixed(1) + " al año)");
console.log("");
console.log("  " + "ETF".padEnd(8) + "coste".padStart(9) + "cuesta al año".padStart(16) + "vs SPY".padStart(12));
const OP = [["SPY", 0.000945], ["IVV", 0.0003], ["VOO", 0.0003], ["SPLG", 0.0002]];
const base = medio * 0.000945;
for (const [n, c] of OP) console.log("  " + n.padEnd(8) + ((100 * c).toFixed(3) + "%").padStart(9) +
  D(medio * c).padStart(16) + (n === "SPY" ? "—" : "ahorras " + D(base - medio * c)).padStart(12 + (n === "SPY" ? 0 : 9)));
console.log("");
console.log("  el contrato mediano de la estrategia cuesta: " + D(16905));
console.log("");
