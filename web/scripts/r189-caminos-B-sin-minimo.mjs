// ══ ¿EL MÍNIMO DE $5.000 APORTA ALGO? ══ Lester, 2026-08-30:
//   «mide esto en los 27. Y me vas a reportar si el éxito es gracias a él, a pesar de él,
//    o independiente de él.»
//
// El $5.000 nació como ETIQUETA de una tabla en r7 («menos de $5.000 / $5.000 a $10.000»)
// y en r135 lo convertí en FILTRO sin medir nada. De ahí se copió hasta la regla congelada.
//
// Este fichero es r137 con UN cambio: COSTE_MIN = 0. Todo lo demás idéntico — mismos
// tickers, mismas cadenas, misma elección de contrato, mismas tolerancias.
// Así `elegir()` coge el contrato más cercano a 25%/400d SIN mirar el precio, y el filtro
// se puede encender y apagar AL MEDIR, sobre las mismas operaciones. Comparar el fichero
// viejo contra el nuevo NO valdría: con el mínimo dentro, elegir() sustituye por un
// contrato más profundo o más largo, así que cambiarían dos cosas a la vez.
...
// TERCERA RONDA — EL DIAL DEL AGUANTE.
// Los ficheros anteriores cortan el camino a los 90 días, así que no se puede medir aguantar
// más sin volver a leer las cadenas. Aquí se guardan **250 días** de camino: con eso se puede
// probar CUALQUIER aguante ≤250 recortando al medir, sin más pasadas.
// Sólo las dos profundidades y los dos plazos que ganaron (25%/35% × 250d/400d).
//
...
const COSTE_MIN = 0, SUELO = 0.50, PLAZO = 130;   // 60 sesiones de aguante + margen   // ← se guardan 250 días de camino
const DESDE = "20160104", HASTA = "20260819";

const cad = abrir("cadenas-B", { callado: true });
...
for (const p of PROFS) for (const dt of DTES) {
  ...
  const f = join(CACHE, "sincosteB-p" + (100*p).toFixed(0) + "-d" + dt + ".json");
