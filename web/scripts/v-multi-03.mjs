// MÁS HUECOS x MÁS DE UNA POSICIÓN POR TICKER, con la EXPOSICIÓN CONGELADA en 24%.
// (tam = 0,24/huecos). Mediana de 41 capitales de partida.
...
function medir(extra){
  ...
  for(let i=0;i<41;i++){const cap=CAP*(1+(i-20)*0.005);
    ...
    // racimo: mayor P&L acumulado de un MISMO ticker en un MISMO episodio (entradas solapadas)
    ...
    for(const tk in byTk){const L=byTk[tk].sort((a,b)=>a.dC<b.dC?-1:1); let acc=0,fin='';
      for(const o of L){ if(fin && o.dC>fin){acc=0;} acc+=o.pnl; if(!fin||(o.dS||'99999999')>fin)fin=o.dS||'99999999'; if(acc>mx)mx=acc; }}
    CL.push(pos>0?100*mx/pos:0);
    if(i===20) globalThis.__t=q.tom;}
  return {d:M.med(F)/M.ANOS,a:M.med(A),c:M.med(C),s:M.med(S),o:M.med(O),may:M.med(MAY),dob:M.med(DOB),cl:M.med(CL)};
}
const f=(x,n=0)=>x.toFixed(n);
console.log(`\n═══ ${U} — huecos x porTicker, exposicion congelada al 24% ═══`);
console.log('huecos  tam   pTk |    $/año   CAGR   caída  Sharpe   ops | mayor%  racimo%  dobl%');
for(const h of [10,14,20,30]) for(const pt of [1,2,3]){
  const r=medir({huecos:h,tam:0.24/h,porTicker:pt});
  console.log(`${String(h).padStart(6)} ${(100*0.24/h).toFixed(2)}% ${String(pt).padStart(5)} | ${('$'+f(r.d)).padStart(8)} ${f(r.a,1).padStart(5)}% ${f(-r.c,1).padStart(6)}% ${f(r.s,2).padStart(6)} ${f(r.o).padStart(5)} | ${f(r.may,1).padStart(5)}% ${f(r.cl,1).padStart(7)}% ${f(r.dob,1).padStart(5)}%`);
}
