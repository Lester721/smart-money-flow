...
// ══ TAMAÑO VARIABLE: APRETAR CUANDO HAY MIEDO ══ Lester, 2026-08-28.
// La idea: NO filtrar por miedo (eso deja sólo 12 operaciones al año y no llena la cuenta), sino
// conservar las ~50 y SUBIR EL TAMAÑO en las que caen en miedo extremo, donde la ventaja es el
// doble (+23,87% contra +11,09%).
// ⚠️ El riesgo evidente: apretar en p90 es apretar justo cuando el mercado está caído. Puede
// disparar la caída. Se mide, no se supone.
CH = new Map(); SP = new Map();
const DD = DIAS_SPY.filter((d) => spotDe("SPY", d) > 0);
const pSPY = new Map(); for (const d of DD) pSPY.set(d, spotDe("SPY", d));
...
function cuenta({ capital, tam, huecos, mult = 1, umbral = 0.90, modo = "spy", hasta = null }) {
  ...
  for (const hoy of dias) {
    ...
    for (const x of (porDia.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      ...
      const t2 = tam * (x.miedo >= umbral ? mult : 1);       // ← el tamaño variable
      const tope = patr * t2;
      ...
      caja -= dinero; ab.push({ ...x, dinero }); tom.push({ ...x, dinero, gana: dinero * (x.mult - 1), apretada: x.miedo >= umbral }); }
...
function banda(op, base = 60000) { const R = [], C = [], M = [], paso = base * 0.0083;
  for (let c = base * 0.917; c <= base * 1.084; c += paso) { const q = cuenta({ ...op, capital: c });
...
console.log("");
console.log("  ══ TAMAÑO VARIABLE: APRETAR EN MIEDO EXTREMO (p90) ══   $60.000 · descansando en SPY");
console.log("");
console.log("  " + "base".padEnd(22) + "  │ " + "TAMAÑO FIJO".padStart(30) + "  │ " + "x1,5 en miedo".padStart(30) + "  │ " + "x2 en miedo".padStart(30));
for (const [h, t] of [[10, 0.08], [8, 0.10], [6, 0.15], [6, 0.10]]) {
  const f = banda({ tam: t, huecos: h, mult: 1 });
  const m15 = banda({ tam: t, huecos: h, mult: 1.5 });
  const m2 = banda({ tam: t, huecos: h, mult: 2 });
  const fmt = (b) => (D(b.dinero) + "  " + b.a.toFixed(1) + "%  −" + b.c.toFixed(0) + "%").padStart(30);
  console.log("  " + (h + " huecos al " + (100 * t).toFixed(0) + "%").padEnd(22) +
    "  │ " + fmt(f) + "  │ " + fmt(m15) + "  │ " + fmt(m2)); }
console.log("  " + "comprar SPY y dormir".padEnd(22) + "  │ " +
  (D(spyRef) + "  " + an(spyRef, 60000).toFixed(1) + "%  −25%").padStart(30));
// el mejor por relación
let mejor = null;
for (const [h, t] of [[10, 0.08], [8, 0.10], [6, 0.15], [6, 0.10]]) for (const m of [1, 1.5, 2]) {
  const b = banda({ tam: t, huecos: h, mult: m });
  if (!mejor || b.a / Math.max(1, b.c) > mejor.r) mejor = { h, t, m, r: b.a / Math.max(1, b.c), ...b }; }
console.log("");
console.log("  ══ AÑO POR AÑO — " + mejor.h + " huecos al " + (100 * mejor.t).toFixed(0) + "%, x" + mejor.m + " en miedo ══");
console.log("");
const q = cuenta({ capital: 60000, tam: mejor.t, huecos: mejor.h, mult: mejor.m });
console.log("  " + "año".padEnd(7) + "valor".padStart(13) + "% del año".padStart(11) + "ops".padStart(6) + "apretadas".padStart(11) + "gana".padStart(7));
let v0 = 60000;
for (const y of ["2018","2019","2020","2021","2022","2023","2024","2025","2026"]) {
  const fin = [...DD].reverse().find((d) => d.startsWith(y)); if (!fin) continue;
  const r = cuenta({ capital: 60000, tam: mejor.t, huecos: mejor.h, mult: mejor.m, hasta: fin });
  ...
  console.log("  " + y.padEnd(7) + D(r.final).padStart(13) +
    ...
    String(del.length).padStart(6) + String(del.filter((x) => x.apretada).length).padStart(11) +
    String(del.filter((x) => x.gana > 0).length).padStart(7));
  v0 = r.final; }
...
console.log("  TOTAL: " + D(q.final) + "  ·  " + an(q.final, 60000).toFixed(1) + "% al año  ·  caída −" + q.caida.toFixed(0) +
  "%  ·  " + q.tom.length + " ops (" + q.tom.filter((x) => x.apretada).length + " apretadas)  ·  acierta " +
  (100 * g / Math.max(1, q.tom.length)).toFixed(0) + "%");
console.log("");
console.log("  ⚠️ El umbral p90 y el multiplicador se eligen mirando estos datos. No están examinados.");
console.log("");
