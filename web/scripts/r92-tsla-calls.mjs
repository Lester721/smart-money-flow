...
const TC = T.filter((x) => x.tk === "TSLA" && x.l === "C");
const TP = T.filter((x) => x.tk === "TSLA" && x.l === "P");
console.log("");
console.log("  ══ AUDIT ══");
console.log("  señales de TSLA: " + T.filter((x) => x.tk === "TSLA").length + "  ·  CALLS: " + TC.length + "  ·  puts: " + TP.length);
console.log("");
console.log("  ══ SÓLO TSLA, SÓLO CALLS ══");
console.log("");
const s = stats(TC);
if (!s) { console.log("  ⛔ n = " + TC.length + " — NO SE PUEDE MEDIR. Hacen falta al menos 3 para una t, y ni con 5 significa nada.");
} else { console.log("  a peso igual: " + s.ret.toFixed(2) + "% por operación · acierta " + s.gana.toFixed(0) + "% · t=" + s.t.toFixed(2)); }
console.log("");
console.log("  las " + TC.length + " operaciones, una por una:");
console.log("  " + "compra".padEnd(10) + "strike".padStart(8) + "vence".padStart(10) + "coste".padStart(11) + "salida".padStart(10) + "resultado".padStart(11));
for (const x of TC.sort((a, b) => a.dC.localeCompare(b.dC)))
  console.log("  " + x.dC.padEnd(10) + String(x.K).padStart(8) + x.exp.padStart(10) + D(x.ask * 100).padStart(11) +
    x.dSal.padStart(10) + (((x.mult - 1) >= 0 ? "+" : "") + (100 * (x.mult - 1)).toFixed(1) + "%").padStart(11));
console.log("");
console.log("  ══ Y EN LA CUENTA ══");
console.log("");
console.log("  " + "".padEnd(30) + "acaba con".padStart(13) + "al año".padStart(10) + "caída".padStart(9) + "ops".padStart(6));
for (const [nom, L] of [["TSLA sólo CALLS", TC], ["TSLA sólo puts", TP], ["TSLA las dos", T.filter((x) => x.tk === "TSLA")],
                        ["los 28 tickers, sólo calls", T.filter((x) => x.l === "C")]]) {
  const b = banda(L, 60000);
  console.log("  " + nom.padEnd(30) + D(60000 * Math.pow(1 + b.a / 100, 5.63)).padStart(13) + (b.a.toFixed(1) + "%").padStart(10) +
    ("−" + b.c.toFixed(0) + "%").padStart(9) + String(b.n).padStart(6)); }
console.log("  " + "comprar SPY y dormir".padEnd(30) + D(134588).padStart(13) + "15.4%".padStart(10) + "−25%".padStart(9) + "1".padStart(6));
console.log("");
