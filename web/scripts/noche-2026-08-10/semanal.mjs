...
console.log('=== POR QUE MURIERON TODAS LAS ESTRATEGIAS ANTERIORES: la horquilla contra la prima ===\n');
console.log('QQQ, cotizaciones reales.  "horquilla" = (ask-bid) / precio medio\n');
console.log('distancia      prima (% del strike)   horquilla (% del precio)   se lleva de la prima');
for (const otm of [0, 0.01, 0.02, 0.03, 0.05, 0.08]) {
  const o = correr('theta-sem', 'QQQ', { otm });
  if (o.length < 10) continue;
  const pr = med(o.map(x => x.prima / x.K)) * 100, hq = med(o.map(x => x.horq)) * 100;
  console.log(`${(otm * 100).toFixed(0).padStart(2)}% fuera        ${pr.toFixed(3).padStart(7)}%              ${hq.toFixed(1).padStart(6)}%                 ${(hq / 2).toFixed(1)}% de la prima`);
}
console.log('\n  -> la horquilla NO crece con la distancia, pero la prima se hunde. Vender opciones');
console.log('     baratas es pagar el mismo peaje por un billete que vale diez veces menos.');
console.log('     Tu 0DTE de QQQ a delta 0,07-0,11 estaba justo en la peor zona.');

for (const SYM of ['QQQ', 'SPY']) {
  console.log(`\n=== ${SYM}: vender put AL DINERO, sostenida a vencimiento, colateral con intereses ===\n`);
  for (const [dir, nom] of [['theta-sem', 'SEMANAL'], ['theta-idx', 'MENSUAL']]) {
    f(`  ${nom} al medio`, correr(dir, SYM));
    f(`  ${nom} al BID (peor caso)`, correr(dir, SYM, { entrada: 'bid' }));
  }
  console.log('  --- y a distintas distancias, semanal ---');
  for (const otm of [0, 0.01, 0.02, 0.03, 0.05]) f(`  semanal ${(otm * 100).toFixed(0)}% fuera`, correr('theta-sem', SYM, { otm }));
  const b = P[SYM]; let pico = 0, dd = 0;
  for (const x of b.filter(x => x.d >= '2020-01-10')) { pico = Math.max(pico, x.c); dd = Math.max(dd, 1 - x.c / pico); }
  const b0 = b.find(x => x.d >= '2020-01-10'), b1 = b[b.length - 1];
  const años = (new Date(b1.d) - new Date(b0.d)) / 365 / 864e5;
  console.log(`  REFERENCIA comprar ${SYM}: ${(((b1.c / b0.c) ** (1 / años) - 1) * 100).toFixed(1)}%/año, caida ${(dd * 100).toFixed(0)}%`);
}

console.log('\n=== semanal al dinero QQQ, año a año ===');
const o = correr('theta-sem', 'QQQ');
const y = new Map(); for (const x of o) { const k = x.rolo.slice(0, 4); if (!y.has(k)) y.set(k, []); y.get(k).push(x); }
console.log('año     semanal QQQ    comprar QQQ');
for (const [k, v] of [...y.entries()].sort()) {
  let e = 1; for (const x of v) e *= (1 + x.ret);
  const q = P.QQQ.filter(b => b.d.startsWith(k));
  console.log(`${k}    ${((e - 1) * 100).toFixed(1).padStart(7)}%      ${((q[q.length - 1].c / q[0].c - 1) * 100).toFixed(1).padStart(7)}%`);
}
