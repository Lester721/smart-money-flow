...
// LOS CAMINOS DEL GRUPO A — para el EXAMEN.
//
// Usa EXACTAMENTE los parámetros de REGLA (EXAMEN-grupo-A.mjs), que se congelaron ANTES de
// bajar un solo byte. Nada se elige aquí: todo viene del fichero congelado.
// Se guardan 250 días de camino para que el aguante de 120 se pueda aplicar al medir.
//
// (cabecera original de r137, de donde sale este código)
...
import { GRUPO_A, REGLA } from "./EXAMEN-grupo-A.mjs";
const TK = GRUPO_A;
const PROFS = [REGLA.profundidad];        // cuánto dentro del dinero
const DTES  = [REGLA.plazo];                       // plazo objetivo
const COSTE_MIN = REGLA.costeMin, SUELO = REGLA.suelo, PLAZO = 250;
...
for (const tk of TK) {
  ...
  for (let i = 20; i < todos.length; i++) {
    ...
    for (const prof of PROFS) for (const dteObj of DTES) {
      ...
      if (Math.abs(L.prof - prof) > prof * REGLA.tolProfundidad) continue;
      if (Math.abs(L.dte - dteObj) > dteObj * REGLA.tolPlazo) continue;
      const cam = [];
...
for (const p of PROFS) for (const dt of DTES) {
  ...
  const f = join(CACHE, "caminos-A.json");
