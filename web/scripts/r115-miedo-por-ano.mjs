...
console.log("");
console.log("  ══ 🔬 EL CAJÓN DEL MIEDO EXTREMO, PARTIDO POR AÑO ══");
console.log("  (si esto lo pone un solo año, es ese año disfrazado de hallazgo)");
console.log("");
const EX = NS.filter((x) => x.miedo >= 0.90);
console.log("  " + "año".padEnd(8) + "n".padStart(5) + "% por op".padStart(11) + "acierta".padStart(9) + "t".padStart(8) + "doblan".padStart(9) +
  "  │ " + "el resto de ese año".padStart(24));
for (const y of ["2018","2019","2020","2021","2022","2023","2024","2025","2026"]) {
  const L = EX.filter((x) => x.y === y), R = NS.filter((x) => x.y === y && x.miedo < 0.90);
  const s = stats(L), r = stats(R);
  console.log("  " + y.padEnd(8) + String(L.length).padStart(5) +
    (s ? (s.ret.toFixed(2) + "%").padStart(11) : "—".padStart(11)) + (s ? (s.gana.toFixed(0) + "%").padStart(9) : "—".padStart(9)) +
    (s ? s.t.toFixed(2).padStart(8) : "—".padStart(8)) + (s ? (s.dobla.toFixed(1) + "%").padStart(9) : "—".padStart(9)) +
    "  │ " + (r ? (R.length + " ops   " + r.ret.toFixed(1) + "%   " + r.gana.toFixed(0) + "%") : "—").padStart(24)); }
const conM = ["2018","2019","2020","2021","2022","2023","2024","2025","2026"]
  .map((y) => ({ y, s: stats(EX.filter((x) => x.y === y)) })).filter((x) => x.s);
console.log("  → años con al menos 3 operaciones de miedo extremo: " + conM.length +
  "  ·  positivos: " + conM.filter((x) => x.s.ret > 0).length);
console.log("");
console.log("  ── ¿y si se quita el mejor año? ──");
console.log("");
console.log("  " + "".padEnd(30) + "n".padStart(6) + "% por op".padStart(11) + "acierta".padStart(9) + "t".padStart(8));
const peor = conM.slice().sort((a, b) => b.s.ret - a.s.ret)[0];
fila("miedo extremo, todo", EX);
if (peor) fila("miedo extremo SIN " + peor.y, EX.filter((x) => x.y !== peor.y));
fila("miedo extremo SIN 2020", EX.filter((x) => x.y !== "2020"));
fila("miedo extremo SIN 2020 ni 2026", EX.filter((x) => x.y !== "2020" && x.y !== "2026"));
console.log("");
