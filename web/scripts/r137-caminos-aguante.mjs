...
const DTES  = [250, 400];                       // plazo objetivo
const COSTE_MIN = 5000, SUELO = 0.50, PLAZO = 250;   // ← se guardan 250 días de camino
...
for (const p of PROFS) for (const dt of DTES) {
  ...
  const f = join(CACHE, "largo-p" + (100*p).toFixed(0) + "-d" + dt + ".json");
