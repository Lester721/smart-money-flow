...
// mediana de 41 capitales de partida
function medir(extra, quitar2020 = false) {
  const F=[],A=[],C=[],S=[],O=[],MAY=[],TOT=[];
  for (let i=0;i<41;i++){ const cap = CAP*(1+(i-20)*0.005);
    ...
    const T = quitar2020 ? q.tom.filter(t=>t.y!=='2020') : q.tom;
    const tot = T.reduce((a,t)=>a+t.pnl,0), mx = T.reduce((a,t)=>Math.max(a,t.pnl),0);
    MAY.push(tot>0 ? 100*mx/tot : NaN); TOT.push(tot); }
  return { d: M.med(F)/M.ANOS, a: M.med(A), c: M.med(C), s: M.med(S), o: M.med(O),
           may: (()=>{const Z=MAY.filter(x=>!isNaN(x)); return Z.length?M.med(Z):NaN;})(), tot: M.med(TOT) };
}
const fmt = (r,etq) => `${etq.padEnd(46)} $${Math.round(r.d).toLocaleString('en-US').padStart(7)}/año  ${r.a.toFixed(1).padStart(5)}%  caída ${r.c.toFixed(1).padStart(5)}%  Sh ${r.s.toFixed(3)}  ops ${String(r.o).padStart(4)}  mayor ${isNaN(r.may)?' n/a':r.may.toFixed(1)+'%'} de $${Math.round(r.tot).toLocaleString('en-US')} en opciones`;

const CASOS = JSON.parse(readFileSync(process.argv[2], 'utf8'));
console.log('universo', UNIV, '| ops elegibles', M.OPS.filter(o=>o.ma<0).length, '| años', M.ANOS.toFixed(2));
for (const c of CASOS) console.log(fmt(medir(c.cfg), c.etq));
