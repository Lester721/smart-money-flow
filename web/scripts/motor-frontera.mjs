...
export function simular({ capital = 60000, tam = 0.15, huecos = 6, modo = "spy",
  ...
  let caja = capital, acc = 0, ab = [], tom = [], cerr = [], rechazo = 0, mirados = 0;
  ...
  for (let t = 0; t < dias.length; t++) {
    ...
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) { caja += ab[i].dinero * ab[i].ultMult;
      cerr.push({ tk: ab[i].tk, dC: ab[i].dC, y: ab[i].dC.slice(0,4), dinero: ab[i].dinero,
                  pnl: ab[i].dinero * ab[i].ultMult - ab[i].dinero, multSal: ab[i].ultMult, dSal: hoy });
      ab.splice(i, 1); }
  ...
  return { final, cagr: 100 * (Math.pow(Math.max(final, 1) / capital, 1 / anos) - 1), caida: 100 * peor,
    sharpe: sd > 0 ? (m * 252 - 0.033) / (sd * Math.sqrt(252)) : 0, ops: tom.length, tom, cerr, rechazo, mirados,
    abiertasFin: ab.length,
