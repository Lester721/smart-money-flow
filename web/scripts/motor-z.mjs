...
export function simular({ capital = 60000, tam = 0.15, huecos = 6, modo = "spy",
  ...
  for (let t = 0; t < dias.length; t++) {
    ...
    for (const x of (POR_DIA.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      ...
      if (ab.filter((o) => o.tk === x.tk).length >= porTk) continue;
      ...
      tom.push({ tk: x.tk, dC: x.dC, y: x.dC.slice(0, 4), dinero, iFin,
                 mult: x.camino[iFin][1] * kM, multFin: x.camino[x.camino.length - 1][1] });
      nuevas.push(hoy); }
