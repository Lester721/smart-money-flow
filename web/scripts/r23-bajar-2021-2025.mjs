// 2021-2025 — los cinco años que faltan para validar la tabla mágica, sin HOOD (no tiene cadenas de precios).
// Mismo descargador honesto que enero: volcado del día entero, filtrado sólo por tamaño de la
// operación. Ver r2-bajar-flujo-limpio.mjs y el manifiesto de flujo-limpio/.
//
// NO TOCAR EL TERMINAL MIENTRAS CORRE: las 4 ranuras de la suscripción están ocupadas y una
// quinta petición rompe la sesión (error 478). Eso costó dos relanzamientos el 25 de agosto.
...
const TICKERS = ["AAPL", "AMD", "META", "MSFT", "NVDA", "QQQ", "SPY", "TSLA"];   // sin HOOD
const MESES = [];
for (let a = 2021; a <= 2025; a++) for (let m = 1; m <= 12; m++) MESES.push(String(a) + String(m).padStart(2, "0"));
