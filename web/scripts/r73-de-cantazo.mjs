...
console.log("");
// ¿ENTRAR DE GOLPE O POCO A POCO? — Lester, 2026-08-27.
// rampa = semanas que se tarda en meter el capital inicial en el ETF. 0 = de cantazo.
// Durante la rampa, el dinero que aun no ha entrado se queda al 0%.
function conRampa({ capital = 60000, semanas = 0, etf = "SPY" }) {
  const divD = Math.pow(1 + DIV[etf], 1 / 252) - 1;
  const rampaDias = semanas * 5;
  let caja = capital, acc = 0, ab = [], pico = capital, peor = 0, k = 0;
  const porDia = new Map();
  ...
  for (const hoy of DIAS) {
    ...
    // el techo de lo que puede estar en ETF mientras dura la rampa
    const prog = rampaDias === 0 ? 1 : Math.min(1, k / rampaDias);
    const techo = rampaDias === 0 ? Infinity : capital * prog + Math.max(0, caja + acc * p + inv() - capital);
    if (caja > 0) { const meter = Math.max(0, Math.min(caja, techo - acc * p));
      if (meter > 0) { acc += meter / p; caja -= meter; } }
...
console.log("");
console.log("  ══ AUDIT ══");
const a0 = conRampa({ semanas: 0 }), b0 = cuenta({ etf: "SPY", conDiv: true });
console.log("  rampa 0 semanas = la version normal: " + D(a0.final) + " vs " + D(b0.final) +
  (Math.abs(a0.final - b0.final) < 5 ? "  ✓" : "  ⚠ NO CUADRA"));
console.log("");
console.log("  ══ ¿DE CANTAZO O POCO A POCO? — mediana de 21 capitales ($55k-$65k) ══");
console.log("");
console.log("  " + "cómo entras".padEnd(26) + "al año".padStart(9) + "caída".padStart(9) + "veces que gana".padStart(16));
const REF = [];
for (let c = 55000; c <= 65000.1; c += 500) REF.push(conRampa({ capital: c, semanas: 0 }).final / c);
for (const [nom, s] of [["de cantazo (día 1)", 0], ["en 4 semanas", 4], ["en 12 semanas", 12],
                        ["en 26 semanas (medio año)", 26], ["en 52 semanas (un año)", 52]]) {
  const A = [], C = []; let gana = 0, n = 0;
  for (let c = 55000, i = 0; c <= 65000.1; c += 500, i++) {
    const q = conRampa({ capital: c, semanas: s });
    A.push(100 * (Math.pow(Math.max(q.final, 1) / c, 1 / 5.63) - 1)); C.push(q.caida);
    n++; if (q.final / c >= REF[i]) gana++; }
  console.log("  " + nom.padEnd(26) + (med(A).toFixed(1) + "%").padStart(9) +
    ("−" + med(C).toFixed(0) + "%").padStart(9) +
    (s === 0 ? "—" : gana + " de " + n).padStart(16)); }
console.log("");
