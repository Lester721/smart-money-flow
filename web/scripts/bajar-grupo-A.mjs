...
//
// Se baja SÓLO el grupo A (24 tickers). El B queda intacto para el segundo examen, y así no
// comprometemos 3 horas y 23 GB antes de saber si la hipótesis vive.
//
// ═══ CÓMO ═════════════════════════════════════════════════════════════════════════════════
...
const tareas = [];
for (const tk of GRUPO_A) for (const a of ANIOS) tareas.push([tk, a]);
console.log("");
console.log("  ══ BAJANDO EL GRUPO A ══");
console.log("  " + GRUPO_A.length + " tickers × " + ANIOS.length + " años = " + tareas.length + " llamadas");
console.log("  " + CONC + " en paralelo · ~68 s cada una · estimado " +
  Math.round(tareas.length * 68 / CONC / 60) + " minutos");
console.log("  destino: cache-theta/cadenas-A/   (formato idéntico a cadenas/)");
console.log("");
const t0 = Date.now();
let i = 0, hechas = 0;
