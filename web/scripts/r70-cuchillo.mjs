...
console.log("");
console.log("  ══ EL FILO DEL CUCHILLO — mover el capital de partida $500 ══");
console.log("");
console.log("  si la estrategia es real, empezar con $59.500 o con $60.500 tiene que dar");
console.log("  casi lo mismo. Si salta, el resultado depende de un contrato de más o de menos.");
console.log("");
console.log("  " + "capital".padStart(9) + "   en NADA (limpio)".padStart(22) + "   en SPY".padStart(20) + "   en QQQ".padStart(20));
const R = { NADA: [], SPY: [], QQQ: [] };
for (let c = 55000; c <= 65000.1; c += 500) {
  const fila = ["NADA", "SPY", "QQQ"].map((etf) => {
    const q = cuenta({ etf, capital: c, conDiv: etf !== "NADA" });
    const a = 100 * (Math.pow(Math.max(q.final, 1) / c, 1 / 5.63) - 1);
    R[etf].push(a);
    return (D(q.final) + "  " + a.toFixed(1) + "%").padStart(20); });
  console.log("  " + D(c).padStart(9) + "  " + fila.join(" ")); }
console.log("");
console.log("  ── cuánto BAILA cada versión ──");
console.log("");
for (const etf of ["NADA", "SPY", "QQQ"]) {
  const A = R[etf], m = A.reduce((x, y) => x + y, 0) / A.length;
  const sd = Math.sqrt(A.reduce((s, x) => s + (x - m) * (x - m), 0) / A.length);
  const lo = Math.min(...A), hi = Math.max(...A);
  console.log("  " + (etf === "NADA" ? "en limpio" : "en " + etf).padEnd(12) +
    "media " + m.toFixed(1) + "%   ·   de " + lo.toFixed(1) + "% a " + hi.toFixed(1) +
    "%   ·   baile ±" + sd.toFixed(1) + " puntos" +
    (hi - lo > 6 ? "   ⚠️ CAÓTICO" : "   ✓ estable")); }
console.log("");
