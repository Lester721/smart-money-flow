...
console.log("");
// LA VERSIÓN EN LIMPIO CON EL EFECTIVO AL 3.3% — año por año, las dos cuentas.
// Lester tiene Robinhood Gold, asi que el efectivo ocioso rinde. Sin Gold, rinde 0%.
const TIPO = 0.033;
function limpio({ capital, tipo = TIPO, hasta = DIAS[DIAS.length - 1] }) {
  const intD = Math.pow(1 + tipo, 1 / 252) - 1;
  let caja = capital, ab = [], tom = [], pico = capital, peor = 0, maxFrac = 0;
  let sumaOcioso = 0, nd = 0, interes = 0;
  const porDia = new Map();
  for (const x of T) { if (!porDia.has(x.dC)) porDia.set(x.dC, []); porDia.get(x.dC).push(x); }
  for (const hoy of DIAS.filter((d) => d <= hasta)) {
    const i = caja * intD; caja += i; interes += i;
    for (let k = ab.length - 1; k >= 0; k--) if (ab[k].dSal <= hoy) { caja += ab[k].dinero * ab[k].mult; ab.splice(k, 1); }
    const inv = () => ab.reduce((a, b) => a + b.dinero, 0);
    for (const x of (porDia.get(hoy) || [])) {
      if (ab.length >= 4) continue;
      const patr = caja + inv();
    ...
    const v = caja + inv();
    if (v > pico) pico = v; const dd = 1 - v / pico; if (dd > peor) peor = dd;
    sumaOcioso += caja / v; nd++; }
  let fin = caja;
  for (const x of ab) fin += x.dinero * x.mult;
  return { final: fin, tom, caida: 100 * peor, maxFrac: 100 * maxFrac, ocioso: 100 * sumaOcioso / nd, interes }; }
const an = (f, c) => 100 * (Math.pow(Math.max(f, 1) / c, 1 / 5.63) - 1);
const med = (A) => { const B = [...A].sort((a, b) => a - b); return B[Math.floor(B.length / 2)]; };
console.log("");
console.log("  ══ AUDIT ══");
const c0 = limpio({ capital: 60000, tipo: 0 });
console.log("  con tipo 0% reproduce lo de antes: " + D(c0.final) + " (esperado $203,185)" + (Math.abs(c0.final - 203185) < 3 ? "  ✓" : "  ⚠"));
const solo = limpio({ capital: 60000, tipo: TIPO });
const suma = solo.tom.reduce((a, x) => a + x.dinero * (x.mult - 1), 0);
console.log("  la suma cuadra: $60,000 + " + D(suma) + " opciones + " + D(solo.interes) + " intereses = " +
  D(60000 + suma + solo.interes) + (Math.abs(60000 + suma + solo.interes - solo.final) < 2 ? "  ✓" : "  ⚠"));
console.log("  ¿mira al futuro? " + (T.every((x) => x.dia < x.dC) ? "NO ✓" : "⚠ SÍ"));
console.log("  posición mayor abierta: " + solo.maxFrac.toFixed(1) + "%" + (solo.maxFrac <= 50.5 ? "  ✓ nunca pasa del 50%" : "  ⚠"));
console.log("  el efectivo está ocioso el " + solo.ocioso.toFixed(0) + "% del tiempo — por eso el interés pesa");
for (const cap of [60000, 300000]) {
  const q = limpio({ capital: cap });
  const sinInt = limpio({ capital: cap, tipo: 0 });
  console.log("");
  console.log("  ══════ CUENTA DE " + D(cap) + "  ·  EN LIMPIO, efectivo al 3.3% (Gold) ══════");
  console.log("");
  console.log("  " + "año".padEnd(6) + "señales".padStart(8) + "ops".padStart(5) + "gana".padStart(6) +
    "pierde".padStart(7) + "valor al cierre".padStart(17) + "% del año".padStart(11) + "sin Gold".padStart(11));
  let v0 = cap, w0 = cap;
  for (const [y] of ANOS) {
    const fin = [...DIAS].reverse().find((d) => d.startsWith(y));
    const v1 = limpio({ capital: cap, hasta: fin }).final;
    const w1 = limpio({ capital: cap, tipo: 0, hasta: fin }).final;
    ...
    console.log("  " + y.padEnd(6) + String(T.filter((x) => x.y === y).length).padStart(8) +
      ...
      D(v1).padStart(17) + ((v1 >= v0 ? "+" : "−") + Math.abs(100 * (v1 / v0 - 1)).toFixed(0) + "%").padStart(11) +
      ((w1 >= w0 ? "+" : "−") + Math.abs(100 * (w1 / w0 - 1)).toFixed(0) + "%").padStart(11));
    v0 = v1; w0 = w1; }
  console.log("  " + "-".repeat(72));
  console.log("  CON Gold:  " + D(q.final) + "  ·  " + an(q.final, cap).toFixed(1) + "% al año  ·  caída −" +
    q.caida.toFixed(0) + "%  ·  " + q.tom.length + " ops  ·  " + D(q.interes) + " son intereses");
  console.log("  SIN Gold:  " + D(sinInt.final) + "  ·  " + an(sinInt.final, cap).toFixed(1) + "% al año  ·  caída −" + sinInt.caida.toFixed(0) + "%");
}
console.log("");
console.log("  ── el número honesto: mediana de 21 capitales de partida ──");
for (const [nom, lo, hi] of [["tu cuenta ($55k-$65k)", 55000, 65000], ["la grande ($280k-$320k)", 280000, 320000]]) {
  const A = [], C = [], paso = (hi - lo) / 20;
  for (let c = lo; c <= hi + 0.1; c += paso) { const q = limpio({ capital: c }); A.push(an(q.final, c)); C.push(q.caida); }
  console.log("  " + nom.padEnd(26) + (med(A).toFixed(1) + "% al año").padStart(14) + ("caída −" + med(C).toFixed(0) + "%").padStart(14)); }
console.log("");
console.log("  ⚠️ EN MUESTRA, sobre 8 tickers. El 3.3% es plano: en 2021 el efectivo pagaba ~0% y en 2023-25 ~5%.");
console.log("");
