...
console.log("");
// ¿Y SI GUARDO $5.000 PARA COMPRAR EL HOYO? — Lester, 2026-08-27.
//   "si tuviera $65k en 2021 y meto en el ETF $60k, se cae el mercado y compro el hoyo con los
//    $5k extra en 2022, hubiera salido mejor con SPY o QQQ?"
// La reserva NO financia señales: se queda al 0% hasta que el ETF cae X% desde su máximo.
// El disparador mira SOLO el pasado (el máximo hasta hoy) — no hay mirada al futuro.
function conReserva({ capital = 65000, reserva = 5000, umbral = null, etf = "SPY" }) {
  const divD = Math.pow(1 + DIV[etf], 1 / 252) - 1;
  let res = umbral === null && reserva === 0 ? 0 : reserva;
  let caja = capital - res, acc = 0, ab = [], pico = capital, peor = 0;
  let picoP = 0, diaHoyo = null, precioHoyo = null;
  const porDia = new Map();
  for (const x of T) { if (!porDia.has(x.dC)) porDia.set(x.dC, []); porDia.get(x.dC).push(x); }
  for (const hoy of DIAS) {
    const p = P[etf].get(hoy);
    if (p > picoP) picoP = p;
    acc *= (1 + divD);
    // ¿se dispara la compra del hoyo? (umbral null = nunca)
    if (res > 0 && umbral !== null && 1 - p / picoP >= umbral) {
      acc += res / p; diaHoyo = hoy; precioHoyo = p; res = 0; }
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) { caja += ab[i].dinero * ab[i].mult; ab.splice(i, 1); }
    const inv = () => ab.reduce((a, b) => a + b.dinero, 0);
    for (const x of (porDia.get(hoy) || [])) {
      if (ab.length >= 4) continue;
      const patr = caja + acc * p + inv();                    // la reserva NO cuenta como munición
      const tope = patr * 0.25 * (x.confirma ? 2 : 1);
    ...
    if (v > pico) pico = v; const dd = 1 - v / pico; if (dd > peor) peor = dd; }
  const p = P[etf].get(DIAS[DIAS.length - 1]);
  let fin = caja + acc * p + res;
  for (const x of ab) fin += x.dinero * x.mult;
  return { final: fin, caida: 100 * peor, diaHoyo, precioHoyo }; }
const med = (A) => { const B = [...A].sort((a, b) => a - b); return B[Math.floor(B.length / 2)]; };
const an = (f, c) => 100 * (Math.pow(Math.max(f, 1) / c, 1 / 5.63) - 1);
console.log("");
console.log("  ══ AUDIT ══");
for (const etf of ["SPY", "QQQ"]) {
  const a = conReserva({ capital: 65000, reserva: 0, umbral: null, etf });
  const b = cuenta({ etf, capital: 65000, conDiv: true });
  console.log("  sin reserva = la version normal (" + etf + "): " + D(a.final) + " vs " + D(b.final) +
    (Math.abs(a.final - b.final) < 5 ? "  ✓" : "  ⚠ NO CUADRA")); }
console.log("");
console.log("  ══ $65.000: $60.000 al ETF + $5.000 guardados para el hoyo ══");
console.log("");
console.log("  " + "".padEnd(30) + "SPY".padStart(20) + "QQQ".padStart(20));
console.log("  " + "".padEnd(30) + "al año / caída".padStart(20) + "al año / caída".padStart(20));
const FILAS = [["todo el día 1 (sin reserva)", 0, null], ["reserva que NUNCA se gasta", 5000, null],
  ["compro el hoyo a −10%", 5000, 0.10], ["compro el hoyo a −15%", 5000, 0.15],
  ["compro el hoyo a −20%", 5000, 0.20], ["compro el hoyo a −25%", 5000, 0.25],
  ["compro el hoyo a −30%", 5000, 0.30]];
for (const [nom, r, u] of FILAS) {
  const cel = ["SPY", "QQQ"].map((etf) => {
    const A = [], C = [];
    for (let c = 62000; c <= 70000.1; c += 500) { const q = conReserva({ capital: c, reserva: r, umbral: u, etf });
      A.push(an(q.final, c)); C.push(q.caida); }
    return (med(A).toFixed(1) + "%  −" + med(C).toFixed(0) + "%").padStart(20); });
  console.log("  " + nom.padEnd(30) + cel.join("")); }
console.log("");
console.log("  ── ¿cuándo se habría disparado? ──");
for (const u of [0.10, 0.15, 0.20, 0.25, 0.30]) {
  const f = ["SPY", "QQQ"].map((etf) => { const q = conReserva({ umbral: u, etf });
    return (etf + ": " + (q.diaHoyo || "NUNCA")).padEnd(20); });
  console.log("  a −" + (100 * u).toFixed(0) + "%   " + f.join("")); }
console.log("");
