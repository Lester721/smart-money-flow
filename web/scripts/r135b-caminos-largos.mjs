...
// SEGUNDA RONDA: el plazo de 250d le ganó al de 120d en casi toda la frontera. Y tiene
// mecanismo: aguantamos SÓLO 90 días, así que cuanto más largo el vencimiento MENOS theta
// se paga de la parte que nunca usamos. Aquí se empuja el dial hasta 400 y 550 días,
// sobre las dos profundidades que ganaron (25% y 35%). Así el barrido posterior es instantáneo y no hay que
// volver a leer 1,2 GB por cada combinación.
//
...
const PROFS = [0.25, 0.35];        // cuánto dentro del dinero
const DTES  = [400, 550];                       // plazo objetivo
const COSTE_MIN = 5000, SUELO = 0.50, PLAZO = 90;
...
for (const p of PROFS) for (const dt of DTES) {
  ...
  const f = join(CACHE, "caminos-p" + (100*p).toFixed(0) + "-d" + dt + ".json");
