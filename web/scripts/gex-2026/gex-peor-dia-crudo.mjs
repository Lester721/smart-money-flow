...
function lado(dia,l){const f=`${DIR}/iv_${dia}_${l}.csv`; if(!fs.existsSync(f))return null;
 ...
 for(let n=1;n<lin.length;n++){const c=lin[n].split(','); if(c.length<cab.length)continue;
  ...
  if((ask-bid)/mid>0.5)continue; // filtro normal
...
for(const h of p.horas){
  ...
  if(!c||!cA||!pu||!pA){
    // sin el filtro: ¿EXISTE cotización aunque sea mala? -> distingue "no hay mercado" de
    // "mi filtro la descarta". Es la diferencia entre no poder salir y salir caro.
    const C2=gc.crudo.get(p.Kc),CA2=gc.crudo.get(p.Kc+ALA),P2=gp?.crudo.get(p.Kp),PA2=gp?.crudo.get(p.Kp-ALA);
    if(C2&&CA2&&P2&&PA2&&C2.ask>0&&P2.ask>0){
      const coste=C2.ask+P2.ask-CA2.bid-PA2.bid, pa=coste-p.cr;
      const h2=[C2,CA2,P2,PA2].map(q=>q.mid>0?((q.ask-q.bid)/q.mid*100).toFixed(0)+'%':'—').join('/');
      console.log(`   ${h}  ${gc.U.toFixed(2)}   SÍ existe (horquillas ${h2})   coste $${(coste*100).toFixed(0)}   pérdida $${(pa*100).toFixed(0)}   ${(pa/riesgo*100).toFixed(0)}% del riesgo`);
    } else {
      const falta=[!C2&&'call corta',!CA2&&'call larga',!P2&&'put corta',!PA2&&'put larga'].filter(Boolean).join(', ');
      console.log(`   ${h}  ${gc.U.toFixed(2)}   NO EXISTE cotización: ${falta||'ask a cero'}`);
    }
    continue;}
