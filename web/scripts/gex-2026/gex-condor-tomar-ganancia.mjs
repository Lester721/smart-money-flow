// ¿SOSTENER AL CIERRE O RECOGER LA GANANCIA ANTES?
//
...
const OBJETIVOS = [0.25, 0.50, 0.75];
...
for (const dia of dias) {
  ...
  for (const obj of OBJETIVOS) {
    ...
    for (const h of horas) {
      ...
      if (coste <= credito * (1 - obj)) { salida = { h, coste }; break; }
    }
    const ret = salida
...
console.log(`═══ ¿SOSTENER O RECOGER? — cóndor ±25/alas 50, días GEX+, ${n} operaciones ═══\n`);
console.log('regla                     acierto   media    mediana    t     peor día   \$/año   cerradas antes');
{
  ...
  for (const obj of OBJETIVOS) {
    ...
    console.log(`recoger al ${(obj * 100).toFixed(0)}% del crédito  ${(ss.win * 100).toFixed(0).padStart(3)}%   ${(ss.m * 100).toFixed(2).padStart(6)}%  ${(ss.med * 100).toFixed(2).padStart(7)}%  ${ss.t.toFixed(2).padStart(5)}   ${(ss.peor * 5000).toFixed(0).padStart(7)}   ${(ss.m * 5000 * n / años).toFixed(0).padStart(6)}     ${antes.length} (${(antes.length / g.length * 100).toFixed(0)}%)`);
  }
}
console.log('\n(el \$/año supone 1 contrato y ~55 operaciones al año, riesgo \$5.000)');
console.log('(cerrar antes paga la horquilla DOS veces: 8 patas en vez de 4)');
