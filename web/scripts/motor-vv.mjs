...
export function simular({ capital = 60000, tam = 0.15, huecos = 6, modo = "spy",
                   ...
                   usarRec = false, kv = 0, refv = 0.07, topeMult = 0 } = {}) {
  const fv = (ma) => { if (kv === 0) return 1; let f = 1 + kv * ((-ma) - refv) / refv; if (topeMult > 0) f = Math.min(f, topeMult); return Math.max(0, f); };
  ...
  let caja = capital, acc = 0, ab = [], tom = []; let saltadas = 0; const LIB = [];
  ...
  for (let t = 0; t < dias.length; t++) {
    ...
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) { caja += ab[i].dinero * ab[i].ultMult; LIB.push({tk:ab[i].tk,dC:ab[i].dC,ma:ab[i].ma,dinero:ab[i].dinero,pnl:ab[i].dinero*(ab[i].ultMult-1)}); ab.splice(i, 1); }
    ...
    for (const x of (POR_DIA.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      ...
      const tope = patr * tam * escala * fv(x.ma);
  ...
  return { final, cagr: 100 * (Math.pow(Math.max(final, 1) / capital, 1 / anos) - 1), caida: 100 * peor,
    ...
    invertido: 100 * sInv / V.length, V, R, RB, RS, betaHat, pnlS, pnlO, dias, saltadas,
    LIB: LIB.concat(ab.map(o=>({tk:o.tk,dC:o.dC,ma:o.ma,dinero:o.dinero,pnl:o.dinero*(o.ultMult-1)}))) }; }
