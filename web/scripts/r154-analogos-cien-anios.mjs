...
console.log("");
console.log("  " + "vecinos".padEnd(10) + "umbral".padEnd(10) + "al año".padStart(9) + "caída".padStart(8) +
  "Sharpe".padStart(8) + "dSharpe".padStart(10) + "   ¿bajo el techo +" + techo.toFixed(3) + "?");
const TAB = [];
for (const K of [50, 150, 400, 1000]) {
  for (const umb of [0.02, 0.04]) {
    const r = correr(0.24, reglaAnalogo(K, umb, 0.5, 1.5));
    const d = r.sharpe - base.sharpe;
    TAB.push({ K, umb, d, r });
    console.log("  " + String(K).padEnd(10) + (umb*100).toFixed(0).padEnd(10) + (r.cagr.toFixed(1)+"%").padStart(9) +
      ("−"+r.caida.toFixed(0)+"%").padStart(8) + r.sharpe.toFixed(2).padStart(8) +
      ((d>=0?"+":"")+d.toFixed(3)).padStart(10) + (d > techo ? "         NO, IMPOSIBLE" : "         sí"));
  }
}
console.log("  " + "CONSTANTE".padEnd(20) + (base.cagr.toFixed(1)+"%").padStart(9) +
  ("−"+base.caida.toFixed(0)+"%").padStart(8) + base.sharpe.toFixed(2).padStart(8) + "   ← el listón");
console.log("");
const mej = TAB.slice().sort((a,b)=>b.d-a.d)[0];
console.log("  el mejor: K=" + mej.K + " umbral " + (mej.umb*100).toFixed(0) + "% → " +
  (mej.d>=0?"+":"") + mej.d.toFixed(3) + " de Sharpe");
console.log("  ¿es liso en K? " + TAB.filter(x=>x.umb===mej.umb).map(x=>x.d.toFixed(3)).join(" → "));
console.log("");
if (mej.d > 0.02) {
  console.log("  ── LOS CONTROLES ──");
  const dp = Math.max(...[-25,-10,10,25].map(k => correr(0.24, reglaAnalogo(mej.K, mej.umb, 0.5, 1.5, k)).sharpe - base.sharpe));
  console.log("    desplazado ±25 días : " + (dp>=0?"+":"") + dp.toFixed(3) +
    (mej.d > dp + 0.01 ? "   ✓ la alineada gana" : "   ✗ RUIDO"));
  const inv = correr(0.24, reglaAnalogo(mej.K, mej.umb, 1.5, 0.5)).sharpe - base.sharpe;
  console.log("    señal INVERTIDA     : " + (inv>=0?"+":"") + inv.toFixed(3) +
    (inv < 0 ? "   ✓ el signo importa" : "   ✗ gana igual al revés → RUIDO"));
  console.log("");
  console.log("  " + (mej.d <= techo && mej.d > dp + 0.01 && inv < 0
    ? "⇒ SOBREVIVE. Es la primera regla de régimen que pasa las tres cribas."
    : "⇒ no pasa. " + (mej.d > techo ? "Supera el techo del oráculo: imposible." : "Los controles la tumban.")));
} else {
  console.log("  ⇒ los análogos de cien años NO ganan nada sobre el tamaño constante.");
  console.log("     La idea está medida como es debido y no funciona.");
}
console.log("");
