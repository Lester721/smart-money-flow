...
console.log("");
console.log("  ══ VEREDICTO — mediana de 21 capitales de partida ($55k a $65k) ══");
console.log("");
console.log("  se usa la MEDIANA porque el punto exacto de $60.000 es ruido de");
console.log("  'un contrato más o menos': mover el capital $500 mueve el resultado 4 puntos.");
console.log("");
const med = (A) => { const B = [...A].sort((a, b) => a - b); return B[Math.floor(B.length / 2)]; };
console.log("  " + "".padEnd(22) + "al año".padStart(10) + "caída".padStart(10) + "peor año".padStart(11) +
  "corr. con".padStart(12) + "carry".padStart(9));
console.log("  " + "".padEnd(22) + "(mediana)".padStart(10) + "(mediana)".padStart(10) + "".padStart(11) +
  "las señales".padStart(12) + "neto".padStart(9));
const CORR = { NADA: "—", SPY: "0.868", QQQ: "0.935" };
const CARRY = { NADA: "0.0%", SPY: "+1.2%", QQQ: "+0.4%" };
for (const [nom, etf] of [["en limpio (efectivo 0%)", "NADA"], ["aparcado en SPY", "SPY"], ["aparcado en QQQ", "QQQ"]]) {
  const A = [], C = [], peor = [];
  for (let c = 55000; c <= 65000.1; c += 500) {
    const q = cuenta({ etf, capital: c, conDiv: etf !== "NADA" });
    A.push(100 * (Math.pow(Math.max(q.final, 1) / c, 1 / 5.63) - 1));
    C.push(q.caida);
    // 2022 de esa corrida
    const f21 = [...DIAS].reverse().find((d) => d.startsWith("2021"));
    const f22 = [...DIAS].reverse().find((d) => d.startsWith("2022"));
    const v21 = cuenta({ etf, capital: c, conDiv: etf !== "NADA", L: T.filter((x) => x.dC <= f21), hasta: f21 }).final;
    const v22 = cuenta({ etf, capital: c, conDiv: etf !== "NADA", L: T.filter((x) => x.dC <= f22), hasta: f22 }).final;
    peor.push(100 * (v22 / v21 - 1)); }
  console.log("  " + nom.padEnd(22) + (med(A).toFixed(1) + "%").padStart(10) +
    ("−" + med(C).toFixed(0) + "%").padStart(10) + (med(peor).toFixed(0) + "%").padStart(11) +
    CORR[etf].padStart(12) + CARRY[etf].padStart(9)); }
console.log("");
console.log("  ── el listón, comprar y no tocar (CON dividendos) ──");
for (const etf of ["SPY", "QQQ"]) {
  const solo = 60000 * P[etf].get(DIAS[DIAS.length - 1]) / P[etf].get(DIAS[0]);
  const cd = solo * Math.pow(1 + DIV[etf], 5.63);
  console.log("  " + ("$60.000 en " + etf).padEnd(22) + (anual(cd).toFixed(1) + "%").padStart(10) + D(cd).padStart(12)); }
console.log("");
