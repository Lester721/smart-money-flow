...
const SYM = 'SPXW';
const LEDGER = process.env.TENDENCIA_LEDGER || 'data/forward/condor-tendencia.json';

// ── parámetros pre-registrados. NO TOCAR. ────────────────────────────────────
const HORA = '11:00';
const SEP = 30;   // ±30, no ±25: la rejilla de 6 distancias baja la caída de forma monótona
const ALA = 50;
const COMM = 0.03;
const PASO_STRIKE = 5;
...
const CRED = { p10: 120, p25: 210, p50: 336, p90: 700 };
...
const STORE = (process.env.TENDENCIA_STORE || (process.env.REDIS_URL ? 'redis' : 'file')).toLowerCase();
const REDIS_KEY = process.env.TENDENCIA_REDIS_KEY || 'forward:condor-tendencia';
let _redis = null;
async function redis() {
  if (!_redis) {
    if (!process.env.REDIS_URL) throw new Error('TENDENCIA_STORE=redis pero falta REDIS_URL en el entorno');
...
const guardar = async (l, reporte = '', resumen = '') => {
  if (STORE === 'redis') {
    ...
    await escribirLatido(r, 'condor-tendencia', resumen || `${l.length} operaciones en el ledger`);
    return;
  }
...
async function mediasHastaAyer(dia) {
  ...
  return { ma20: med(20), ma50: med(50), n: cierres.length };
}
...
(async () => {
  ...
  console.log(`\n═══ FORWARD-TEST · cóndor 0DTE + TENDENCIA · ${DIA} ═══`);
  ...
  if (ledger.some(o => o.dia === DIA)) { console.log('    ya estaba registrado — no se duplica'); }
    ...
    if (!f) { console.log(`    ✗ sin datos para ${DIA} (¿festivo? ¿aún no son las ${HORA}?)`); }
      ...
      } else if (!(f.U >= mm.ma20 && f.U >= mm.ma50)) {
        ledger.push({ ...base, ma20: Math.round(mm.ma20 * 100) / 100, ma50: Math.round(mm.ma50 * 100) / 100,
                      estado: 'sin señal', motivo: 'SPX por debajo de su MA20 y/o MA50 — el filtro' });
        console.log('    SPX ' + f.U.toFixed(2) + ' · MA20 ' + mm.ma20.toFixed(2) + ' · MA50 ' + mm.ma50.toFixed(2) + '  ->  NO OPERAR (por debajo)');
      } else {
        base.ma20 = Math.round(mm.ma20 * 100) / 100;
  ...
  console.log(`\n[3] estado del ledger`);
  di(`    días registrados: ${ledger.length}  ·  con señal: ${señales.length} (${ledger.length ? Math.round(señales.length / ledger.length * 100) : 0}%, con el filtro debe rondar el 70%)`);
  if (cer.length) {
    ...
    di(`    cerradas: ${cer.length}  ·  acierto ${Math.round(gan.length / cer.length * 100)}% (backtest 87%)`);
    di(`    crédito mediano: $${credMed.toFixed(0)}  (últimos 12 meses del backtest: p10 $${CRED.p10} · mediana $${CRED.p50} · p90 $${CRED.p90})`);
    di(`    P&L acumulado: $${tot.toFixed(0)}  ·  por operación $${(tot / cer.length).toFixed(0)} (backtest $109)`);
...
// que dijo" en vez de callarse. El error se relanza después: el cron tiene que seguir fallando.
  ...
  try {
    ...
    await escribirLatidoDirecto('condor-tendencia', `FALLÓ: ${e?.message ?? e}`);
