...
export function simular({ capital = 60000, tam = 0.15, huecos = 6, modo = "spy",
                   ...
                   usarRec = false,
                   kProf = 0, topeMult = 0, pisoMult = 0, refProf = 0.07, formaProf = "lineal", umbralProf = 0.15 } = {}) {
  // ══ TAMAÑO SEGÚN LA PROFUNDIDAD (copia de motor-cartera.mjs, NO tocar el original) ══
  //   f = 1 + kProf · (|ma| − refProf)/refProf   (recortada a [pisoMult, topeMult])
  //   El tamaño de CADA posición se multiplica por f. `tam` se recalibra fuera para que
  //   la exposición media (q.invertido) siga siendo la misma que la regla plana: si no,
  //   se estaría comparando una cartera más grande contra otra más pequeña.
  const fProfDe = (ma) => {
    if (kProf === 0) return 1;
    const prof = (-ma - refProf) / refProf;         // 0 en el umbral, 3,29 en −30%
    let f = formaProf === "raiz" ? 1 + kProf * Math.sqrt(Math.max(0, prof))
          : formaProf === "cuad" ? 1 + kProf * prof * prof
          : formaProf === "escalon" ? (-ma >= (umbralProf || 0.15) ? kProf : 1)
          : 1 + kProf * prof;
    if (topeMult > 0) f = Math.min(f, topeMult);
    if (pisoMult > 0) f = Math.max(f, pisoMult);
    return Math.max(0, f); };
  ...
  let caja = capital, acc = 0, ab = [], tom = [], cerradas = [];
  ...
  for (let t = 0; t < dias.length; t++) {
    ...
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) {
      ...
      cerradas.push({ tk: ab[i].tk, dC: ab[i].dC, dS: hoy, dinero: ab[i].dinero,
                      multSal: ab[i].ultMult, pnl: ab[i].dinero * (ab[i].ultMult - 1), ma: ab[i].ma });
      ab.splice(i, 1); }
    ...
    for (const x of (POR_DIA.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      ...
      const tope = patr * tam * escala * fProfDe(x.ma);
  ...
  return { final, cagr: 100 * (Math.pow(Math.max(final, 1) / capital, 1 / anos) - 1), caida: 100 * peor,
    ...
    invertido: 100 * sInv / V.length, V, R, RB, RS, betaHat, pnlS, pnlO, dias, cerradas,
    abiertasFin: ab.map(o => ({ tk: o.tk, dC: o.dC, dinero: o.dinero, multSal: o.ultMult,
                                pnl: o.dinero * (o.ultMult - 1), ma: o.ma })) }; }
