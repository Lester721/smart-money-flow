...
console.log("");
console.log("  ══ ¿ES ESTABLE? — barrido del dividendo, en pasos diminutos ══");
console.log("");
console.log("  un dividendo mayor SIEMPRE debería dar MÁS dinero. Si salta, el resultado es caótico.");
console.log("");
for (const etf of ["SPY", "QQQ"]) {
  console.log("  " + etf + ":");
  let ant = null, saltos = 0, n = 0;
  for (let d = 0; d <= 0.020001; d += 0.001) {
    DIV[etf] = d;
    const q = cuenta({ etf, conDiv: true });
    const flecha = ant == null ? "" : (q.final > ant ? "↑" : q.final < ant ? "↓ ¡BAJA!" : "=");
    if (ant != null) { n++; if (q.final < ant) saltos++; }
    console.log("    dividendo " + (100 * d).toFixed(1) + "%  →  " + D(q.final) +
      "   " + anual(q.final).toFixed(1) + "%   " + flecha);
    ant = q.final; }
  console.log("    → " + saltos + " de " + n + " pasos van AL REVÉS." +
    (saltos > 0 ? "  ⚠️ el resultado es CAÓTICO, no una medición." : "  ✓ monótono"));
  console.log(""); }
