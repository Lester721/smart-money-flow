...
const SYM = 'SPXW';
const LEDGER = process.env.SINFILTRO_LEDGER || 'data/forward/condor-sinfiltro.json';
...
const CRED = { p10: 220, p25: 355, p50: 500, p90: 1175 };
...
const STORE = (process.env.SINFILTRO_STORE || (process.env.REDIS_URL ? 'redis' : 'file')).toLowerCase();
const REDIS_KEY = process.env.SINFILTRO_REDIS_KEY || 'forward:condor-sinfiltro';
let _redis = null;
async function redis() {
  if (!_redis) {
    if (!process.env.REDIS_URL) throw new Error('SINFILTRO_STORE=redis pero falta REDIS_URL en el entorno');
...
const guardar = async (l, reporte = '', resumen = '') => {
  if (STORE === 'redis') {
    ...
    await escribirLatido(r, 'condor-sinfiltro', resumen || `${l.length} operaciones en el ledger`);
    return;
  }
...
(async () => {
  ...
  console.log(`\n═══ FORWARD-TEST · cóndor 0DTE SIN FILTRO · ${DIA} ═══`);
  ...
  if (ledger.some(o => o.dia === DIA)) { console.log('    ya estaba registrado — no se duplica'); }
    ...
    if (!f) { console.log(`    ✗ sin datos para ${DIA} (¿festivo? ¿aún no son las ${HORA}?)`); }
      ...
      if (false) {   // SIN VETO — el GEX se calcula y se guarda, pero no decide. Ver la cabecera.
  ...
  console.log(`\n[3] estado del ledger`);
  di(`    días registrados: ${ledger.length}  ·  con señal: ${señales.length} (${ledger.length ? Math.round(señales.length / ledger.length * 100) : 0}%, sin filtro debe ser 100%)`);
  if (cer.length) {
    ...
    di(`    cerradas: ${cer.length}  ·  acierto ${Math.round(gan.length / cer.length * 100)}% (backtest 75%)`);
    di(`    crédito mediano: $${credMed.toFixed(0)}  (últimos 12 meses del backtest: p10 $${CRED.p10} · mediana $${CRED.p50} · p90 $${CRED.p90})`);
    di(`    P&L acumulado: $${tot.toFixed(0)}  ·  por operación $${(tot / cer.length).toFixed(0)} (backtest $74)`);
...
// que dijo" en vez de callarse. El error se relanza después: el cron tiene que seguir fallando.
  ...
  try {
    ...
    await escribirLatidoDirecto('condor-sinfiltro', `FALLÓ: ${e?.message ?? e}`);
