...
const SECTOR = { AAPL:"tec", AMD:"tec", MSFT:"tec", NVDA:"tec", INTC:"tec", CSCO:"tec", ORCL:"tec",
  ...
  WBA:"sal", T:"tel",
  // ── grupo A ──
  ABBV:"sal", ABT:"sal", AIG:"fin", AMAT:"tec", AMGN:"sal", AVGO:"tec", AXP:"fin", C:"fin",
  CVX:"ene", DAL:"ind", DE:"ind", EBAY:"con", HON:"ind", LLY:"sal", LMT:"ind", LRCX:"tec",
  MDT:"sal", PEP:"con", PG:"con", SCHW:"fin", SLB:"ene", TGT:"con", YUM:"con", ZTS:"sal",
  // ── grupo B ──
  ACN:"tec", ADBE:"tec", AMZN:"con", BKNG:"con", BMY:"sal", CAT:"ind", CVS:"sal", DHR:"sal",
  FDX:"ind", GILD:"sal", GM:"con", GOOGL:"tec", GS:"fin", HD:"con", IBM:"tec", JNJ:"sal",
  LOW:"con", MA:"fin", MCD:"con", MRK:"sal", MS:"fin", MU:"tec", NFLX:"con", QCOM:"tec",
  RTX:"ind", SBUX:"con", SO:"uti", TMO:"sal", TXN:"tec", UPS:"ind", USB:"fin", V:"fin",
  VZ:"tel", WFC:"fin", X:"mat", XLNX:"tec" };
...
export function simular({ capital = 60000, tam = 0.15, huecos = 6, modo = "spy",
                   ...
                   usarRec = false,
                   volVent = 60, escMin = 0.20, ddFreno = 0, ddReent = 0, pesoSPY = 1, frenoEsc = 0 } = {}) {
  ...
  let paradoDD = false;              // freno: no abrir mientras la CUENTA esté hundida
  ...
  for (let t = 0; t < dias.length; t++) {
    ...
    if (modo === "spy") { acc *= (1 + divD); caja *= (1 + intD); } else caja *= (1 + intD);
    ...
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) {
      caja += ab[i].dinero * ab[i].ultMult;
      if (ab[i].reg) { ab[i].reg.multR = ab[i].ultMult; ab[i].reg.pnl = ab[i].dinero * (ab[i].ultMult - 1); }
      ab.splice(i, 1); }
    ...
    let escala = 1;
    if (volObj > 0 && V.length >= volVent) {
      const r = [];
      for (let i = V.length - volVent + 1; i < V.length; i++) r.push(V[i] / V[i - 1] - 1);
      const m = r.reduce((a, x) => a + x, 0) / r.length;
      const sd = Math.sqrt(r.reduce((a, x) => a + (x - m) ** 2, 0) / (r.length - 1)) * Math.sqrt(252);
      escala = Math.max(escMin, Math.min(1, volObj / Math.max(0.01, sd))); }
    // ── FRENO POR CAIDA DE LA PROPIA CUENTA: no abrir mientras el patrimonio este hundido
    if (ddFreno > 0 && V.length) { const ddH = 1 - V[V.length - 1] / pico;
      if (!paradoDD && ddH >= ddFreno) paradoDD = true;
      else if (paradoDD && ddH <= (ddReent > 0 ? ddReent : ddFreno)) paradoDD = false; }

    // ── 5. abrir ──
    if ((parado || paradoDD) && frenoEsc > 0) escala *= frenoEsc;   // freno blando: se abre MENOS
    const corte = dias[Math.max(0, t - 21)];
    for (const x of (POR_DIA.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      if (ab.length >= huecos) break;
      if ((parado || paradoDD) && frenoEsc <= 0) break;   // freno duro: no se abre nada
      ...
      const reg = { tk: x.tk, dC: x.dC, y: x.dC.slice(0, 4), dinero, mult: x.camino[x.camino.length - 1][1],
                    multR: kM, pnl: 0 };
      ab.push({ ...x, dinero, ultMult: kM, dSal: nS, reg });
      tom.push(reg);
      nuevas.push(hoy); }

    if (modo === "spy") { if (pesoSPY >= 1) { if (caja > 0) { acc += caja / p; caja = 0; } }
      else { const ocio = caja + acc * p; if (ocio > 0) { const obj = ocio * pesoSPY; acc = obj / p; caja = ocio - obj; } } }
    const libro = ab.reduce((a, o) => a + o.dinero * o.ultMult, 0);
  ...
  for (const o of ab) if (o.reg) { o.reg.multR = o.ultMult; o.reg.pnl = o.dinero * (o.ultMult - 1); }
