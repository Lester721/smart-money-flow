...
export function simular({ capital = 60000, tam = 0.15, huecos = 6, modo = "spy",
                   ...
                   usarRec = false,
                   // ── AÑADIDOS DE ULTRACODE (motor-cartera.mjs NO se ha tocado) ──
                   porTicker = 1,       // posiciones abiertas simultáneas del mismo ticker
                   sepDias = 0,         // sesiones mínimas entre dos entradas del mismo ticker
                   kProf = 0,           // escalado del tamaño por profundidad de la caída
                   umbralProf = 0.07,   // ancla del escalado (ahí el multiplicador vale 1)
                   topeProf = 0,        // tope del multiplicador de profundidad (0 = sin tope)
                   kBajo = 1, kAlto = 1 // tamaño según SPY esté bajo / sobre su MA50 (x.sbaj)
                   } = {}) {
  ...
  let caja = capital, acc = 0, ab = [], tom = [], cerr = [];
  const ultEnt = new Map();   // ticker -> índice de sesión de su última entrada
  ...
  for (let t = 0; t < dias.length; t++) {
    ...
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) {
      const v = ab[i].dinero * ab[i].ultMult; caja += v;
      cerr.push({ tk: ab[i].tk, dC: ab[i].dC, pnl: v - ab[i].dinero, dinero: ab[i].dinero });
      ab.splice(i, 1); }
    ...
    for (const x of (POR_DIA.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      ...
      if (ab.filter((o) => o.tk === x.tk).length >= porTicker) continue;
      if (sepDias > 0 && ultEnt.has(x.tk) && t - ultEnt.get(x.tk) < sepDias) continue;
      ...
      // ── ULTRACODE: el tamaño de ESTA posición. Dos multiplicadores, ambos 1 por defecto.
      let fx = 1;
      if (kProf > 0) { const prof = -x.ma;      // x.ma es negativo cuando es elegible
        let mp = 1 + kProf * (prof - umbralProf) / umbralProf;
        if (topeProf > 0 && mp > topeProf) mp = topeProf;
        if (mp < 0.05) mp = 0.05;
        fx *= mp; }
      if (kBajo !== 1 || kAlto !== 1) fx *= (x.sbaj ? kBajo : kAlto);
      const tope = patr * tam * escala * fx;
  ...
  return { final, cagr: 100 * (Math.pow(Math.max(final, 1) / capital, 1 / anos) - 1), caida: 100 * peor,
    sharpe: sd > 0 ? (m * 252 - 0.033) / (sd * Math.sqrt(252)) : 0, ops: tom.length, tom, cerr,
    abiertas: ab.map((o) => ({ tk: o.tk, dC: o.dC, pnl: o.dinero * o.ultMult - o.dinero, dinero: o.dinero })),
