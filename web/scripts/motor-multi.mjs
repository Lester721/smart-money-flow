// ══ MOTOR DE CARTERA — COPIA con MÁS DE UNA POSICIÓN POR TICKER ══
// Copia literal de motor-cartera.mjs (NO se toca el original: lo usan 400 scripts).
// Lo único que cambia: el veto «una sola posición por acción» pasa a ser dos mandos,
//   porTicker  = cuántas posiciones simultáneas se permiten en la MISMA acción (0 = sin límite)
//   sepDias    = sesiones mínimas entre dos entradas de la misma acción
// y cada operación guarda su P&L realizado, para poder medir concentración y correlación.
// ══ (original) extraído de r120 SIN TOCAR NADA, para que r120 y r121
...
export function simular({ capital = 60000, tam = 0.15, huecos = 6, modo = "spy",
                   ...
                   porTicker = 1, sepDias = 0, dobleSiPeor = null,
                   usarRec = false } = {}) {
  ...
  const ultEnt = new Map();   // ticker -> índice t de su última entrada (para `sepDias`)
  ...
  for (let t = 0; t < dias.length; t++) {
    ...
    for (let i = ab.length - 1; i >= 0; i--) if (ab[i].dSal <= hoy) { caja += ab[i].dinero * ab[i].ultMult;
      ab[i].rec.pnl = ab[i].dinero * (ab[i].ultMult - 1); ab[i].rec.dS = hoy; ab.splice(i, 1); }
    ...
    for (const x of (POR_DIA.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      ...
      // ── EL VETO, ahora con dial. `porTicker` posiciones simultáneas como mucho en la
      //    misma acción; `sepDias` sesiones mínimas entre dos entradas del mismo ticker.
      if (porTicker > 0 && ab.reduce((a, o) => a + (o.tk === x.tk ? 1 : 0), 0) >= porTicker) continue;
      if (sepDias > 0) { const u = ultEnt.get(x.tk); if (u != null && t - u < sepDias) continue; }
      // `dobleSiPeor`: para DOBLAR hay que estar comprando una caída MÁS PROFUNDA que la que
      //   ya se compró en ese ticker (ma nueva ≤ ma abierta − margen). Promediar a la baja de
      //   verdad, no repetir la misma señal cuatro días seguidos.
      if (dobleSiPeor != null) { let peorAb = null;
        for (const o of ab) if (o.tk === x.tk && (peorAb == null || o.ma < peorAb)) peorAb = o.ma;
        if (peorAb != null && !(x.ma <= peorAb - dobleSiPeor)) continue; }
      ...
      // cuántas posiciones de ESTE ticker ya estaban abiertas al entrar: 0 = plaza limpia,
      // 1 o más = estamos DOBLANDO. Sirve para separar después el dinero de las dobladas.
      const yaAbiertas = ab.reduce((a, o) => a + (o.tk === x.tk ? 1 : 0), 0);
      const rec = { tk: x.tk, dC: x.dC, y: x.dC.slice(0, 4), dinero,
                    mult: x.camino[x.camino.length - 1][1], dobla: yaAbiertas, pnl: 0, dS: null };
      ab.push({ ...x, dinero, ultMult: kM, dSal: nS, rec });
      tom.push(rec);
      ultEnt.set(x.tk, t);
      nuevas.push(hoy); }
  ...
  // las que siguen abiertas el último día: su P&L se marca a mercado, no se inventa salida
  for (const o of ab) { o.rec.pnl = o.dinero * (o.ultMult - 1); o.rec.abierta = true; }
