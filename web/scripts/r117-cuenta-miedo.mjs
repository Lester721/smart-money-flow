...
// ══ LA CUENTA, SÓLO CON MIEDO EXTREMO ══ Lester, 2026-08-28.
// Sin el filtro del 5%: son casi excluyentes (de 108 entradas de miedo, sólo 8 con SPY sano).
CH = new Map(); SP = new Map();
...
function cuenta({ L, capital, tam, huecos, modo = "spy", hasta = null }) {
  ...
  for (const hoy of dias) {
    ...
    for (const x of (porDia.get(hoy) || []).slice().sort((a, b) => a.ma - b.ma)) {
      ...
      const patr = caja + acc * p + inv(), tope = patr * tam;
...
const MIEDO_OPS = OPS.filter((x) => x.miedo >= 0.90);
const spyRef = 60000 * (pSPY.get(DD[DD.length - 1]) / pSPY.get(DD[0])) * Math.pow(1 + DIV_SPY, ANOS);
console.log("");
console.log("  ══ LA CUENTA, SÓLO CON MIEDO EXTREMO ══   $60.000 · 2016-2026 (" + ANOS.toFixed(1) + " años)");
console.log("  (" + MIEDO_OPS.length + " señales brutas · " + (MIEDO_OPS.length / ANOS).toFixed(1) + " al año)");
console.log("");
console.log("  " + "huecos".padEnd(8) + "por posic.".padStart(11) + "  │ " + "DESCANSANDO EN SPY".padStart(32) +
  "  │ " + "SOLA (efectivo 3,3%)".padStart(30) + "ops".padStart(6));
const RES = [];
for (const [h, t] of [[4, 0.25], [4, 0.15], [6, 0.15], [8, 0.10], [10, 0.08], [3, 0.33]]) {
  const s2 = banda({ L: MIEDO_OPS, tam: t, huecos: h, modo: "spy" });
  const e2 = banda({ L: MIEDO_OPS, tam: t, huecos: h, modo: "efectivo" });
  const q = cuenta({ L: MIEDO_OPS, capital: 60000, tam: t, huecos: h, modo: "spy" });
  RES.push({ h, t, s: s2, ops: q.tom.length, inv: q.invertido });
  console.log("  " + String(h).padEnd(8) + ((100 * t).toFixed(0) + "%").padStart(11) +
    "  │ " + (D(s2.dinero) + "   " + s2.a.toFixed(1) + "%   −" + s2.c.toFixed(0) + "%").padStart(32) +
    "  │ " + (D(e2.dinero) + "   " + e2.a.toFixed(1) + "%   −" + e2.c.toFixed(0) + "%").padStart(30) +
    String(q.tom.length).padStart(6)); }
console.log("  " + "comprar SPY y dormir".padEnd(19) + "  │ " + (D(spyRef) + "   " + an(spyRef, 60000).toFixed(1) + "%   −25%").padStart(32));
console.log("  " + "SIN filtro de miedo (todo)".padEnd(19) + "  │ " +
  (() => { const b = banda({ L: OPS, tam: 0.08, huecos: 10, modo: "spy" });
    return (D(b.dinero) + "   " + b.a.toFixed(1) + "%   −" + b.c.toFixed(0) + "%").padStart(32); })());
const mejor = RES.filter((r) => r.ops >= 40).sort((a, b) => (b.s.a / Math.max(1, b.s.c)) - (a.s.a / Math.max(1, a.s.c)))[0] || RES[0];
for (const [nom, modo] of [["DESCANSANDO EN SPY", "spy"], ["SOLA (efectivo 3,3%)", "efectivo"]]) {
  const q = cuenta({ L: MIEDO_OPS, capital: 60000, tam: mejor.t, huecos: mejor.h, modo });
  console.log("");
  console.log("  ── " + mejor.h + " huecos al " + (100 * mejor.t).toFixed(0) + "% · " + nom + " ──");
  ...
  for (const y of ["2018","2019","2020","2021","2022","2023","2024","2025","2026"]) {
    ...
    const r = cuenta({ L: MIEDO_OPS.filter((x) => x.dC <= fin), capital: 60000, tam: mejor.t, huecos: mejor.h, modo, hasta: fin });
...
console.log("");
console.log("  ⚠️ El umbral del miedo (p90) se eligió mirando estos datos. Y 2 de los 5 años con");
console.log("     miedo extremo (2018 y 2022) NO funcionan.");
console.log("");
