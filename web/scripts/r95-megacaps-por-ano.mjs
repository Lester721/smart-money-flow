...
const MEGA = ["GOOGL","AMZN","AVGO","MU","MRVL","TSM","PLTR","ASML","MSTR","DELL","WDC","AMAT","STX","GS","NFLX","COIN","BABA","SHOP","UBER","ARM"];
...
console.log("");
console.log("  ══ AUDIT ══");
console.log("  señales en los 20 megacaps: " + M.length + " (esperado 57)" + (M.length === 57 ? "  ✓" : "  ⚠") +
  "  ·  CALLS " + M.filter((x) => x.l === "C").length + " · puts " + M.filter((x) => x.l === "P").length);
const s = stats(M);
console.log("  a peso igual: " + s.ret.toFixed(2) + "% · acierta " + s.gana.toFixed(0) + "% · t=" + s.t.toFixed(2) + "  (esperado -4.05 / 46 / -1.11)");
console.log("  ¿mira al futuro? " + (M.every((x) => x.dia < x.dC) ? "NO ✓" : "⚠ SÍ"));
console.log("");
console.log("  ══ LOS 20 MEGACAPS, AÑO POR AÑO ══");
console.log("");
...
console.log("");
console.log("  ══ Y EN LA CUENTA (sólo megacaps, $60,000, efectivo al 3.3%) ══");
console.log("");
